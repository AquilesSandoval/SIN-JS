function confirmDelete(key, token) {
    alertify.confirm(
        ...
        function() {
            $.ajax({
                type: 'POST',
                url: "index.php?r=exercisesfolder/delete",
                data: {
                    key: key,
                    token: token
                },
...
jQuery(function ($) {
jQuery('#w0').yiiGridView({"filterUrl":"\/web\/index.php?r=exercisesfolder\/index","filterSelector":"#w0-filters input, #w0-filters select","filterOnFocusOut":true});
});
